...
class CPUProfileImporter {
  ...
  dataFromUrl (myUrl) {
    ...
    if (!match) {
      throw new Error('Invalid URL format, should be like https://bucket.s3.amazonaws.com/brave/browser-laptop/15234--a1b2c3d4e5d6/startup--navigate-manually--100-tabs-2017-09-28T23%3A39%3A49.451Z.cpuprofile')
    }
    const repo = match[1]
    const travisRunId = match[2]
    const gitHash = match[3]
    const fileData = this.dataFromFilename(match[4])
