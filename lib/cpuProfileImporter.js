...
const DATA_FROM_URL_PATHNAME_REGEX = /^\/(?:(.+)\/)?([0-9a-zA-Z\-]+)\/([0-9a-zA-Z.\-%]+)/
...
class CPUProfileImporter {
  ...
  /**
   * @param {string} url https://bucket.s3.amazonaws.com/brave/browser-laptop/15234/startup--navigate-manually--100-tabs-2017-09-28T23%3A39%3A49.451Z.cpuprofile
   * @returns {object}
   */
  ...
  dataFromUrl (myUrl) {
    ...
    // '/brave/browser-laptop/15234/startup--navigate-manually--100-tabs-2017-09-28T23%3A39%3A49.451Z.cpuprofile'
    const match = parsedUrl.pathname.match(DATA_FROM_URL_PATHNAME_REGEX)
    if (!match) {
      throw new Error('Invalid URL format, should be like https://bucket.s3.amazonaws.com/brave/browser-laptop/15234/startup--navigate-manually--100-tabs-2017-09-28T23%3A39%3A49.451Z.cpuprofile')
    }
    const repo = match[1]
    const tag = match[2]
    const gitHashMatch = tag.match(GIT_HASH_REGEX)
    const gitHash = gitHashMatch && gitHashMatch[1]
    let travisRunId = undefined
    if (!gitHash) {
      const travisRunIdMatch = tag.match(TRAVIS_RUN_ID_REGEX)
      travisRunId = travisRunIdMatch && travisRunIdMatch[1]
    }
    const fileData = this.dataFromFilename(match[3])
    const data = Object.assign({repo, gitHash, travisRunId}, fileData)
    return data
  }
